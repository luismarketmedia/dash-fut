    ...
    return (
      <div className="container py-10">
        <div className="rounded-lg border bg-card p-6 text-center">
          Jogo não encontrado. <Link href="/" className="text-primary underline">Voltar</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-6">
        <div className="grid gap-6 md:grid-cols-2">
          <TeamColumn title={`${leftTeam.name}`} color={leftTeam.color} players={leftPlayers} matchId={match.id} update={(pid, fn) => updatePlayerStat(match.id, pid, fn)} />
          <TeamColumn title={`${rightTeam.name}`} color={rightTeam.color} players={rightPlayers} matchId={match.id} update={(pid, fn) => updatePlayerStat(match.id, pid, fn)} />
        </div>
      </div>
    </div>
  );
    ...
    <Card className="overflow-hidden border-0 shadow-sm">
      ...
      <CardContent>
        <div className="space-y-3">
          {players.length === 0 && (
            <div className="rounded-md border p-4 text-sm text-muted-foreground">Sem escalação para este time. Fa��a o sorteio.</div>
          )}
          {players.map((p: any) => {
            const s = (events as any)[p.id] || { goals: 0, yellow: 0, red: false, destaque: false };
            return (
              <div key={p.id} className="flex items-center justify-between gap-3 rounded-md border p-3">
                <div className="flex items-center gap-3">
                  <Input type="number" className="h-8 w-16 text-center" value={p.jerseyNumber} onChange={(e) => dispatch({ type: "UPDATE_PLAYER", payload: { ...p, jerseyNumber: parseInt(e.target.value || "0") } })} />
                  <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-1">
                    <Button size="icon" variant="ghost" onClick={() => update(p.id, (prev: any) => ({ ...prev, goals: Math.max(0, prev.goals - 1) }))}>
                    ...
                    <Button size="icon" variant="ghost" onClick={() => update(p.id, (prev: any) => ({ ...prev, goals: prev.goals + 1 }))}>
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button size="icon" variant="ghost" onClick={() => update(p.id, (prev: any) => ({ ...prev, yellow: Math.max(0, prev.yellow - 1) }))}>
                      <Minus className="h-4 w-4" />
                    </Button>
                    ...
                    <Button size="icon" variant="ghost" onClick={() => update(p.id, (prev: any) => ({ ...prev, yellow: prev.yellow + 1 }))}>
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
