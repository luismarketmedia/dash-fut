...
const POSITIONS: Position[] = ["GOL", "FIXO", "MEIO", "ALA DIREITA", "ALA ESQUERDA", "FRENTE"];
...
function PlayerForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    dispatch({
      type: "ADD_PLAYER",
      payload: { jerseyNumber: Number(jerseyNumber) || 0, name: name.trim(), position, paid },
    });
    setJerseyNumber(0);
    setName("");
    setPosition("GOL");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 rounded-lg border bg-card p-4 shadow-sm">
      <div className="grid gap-3">
        ...
        <Input type="number" min={0} value={jerseyNumber} onChange={(e) => setJerseyNumber(parseInt(e.target.value || "0"))} />
      </div>
      <div className="grid gap-3">
  ...
  return (
    <div className="rounded-lg border bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por nome, número ou posição" />
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.map((p) => (
              <PlayerRow key={p.id} player={p} onDelete={() => dispatch({ type: "DELETE_PLAYER", payload: { id: p.id } })} />
            ))}
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-sm text-muted-foreground">Nenhum jogador</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
...
function PlayerRow({ player, onDelete }: { player: Player; onDelete: () => void }) {
  ...
  const paidIcon = player.paid ? <CheckCircle2 className="h-4 w-4 text-emerald-500" /> : <Circle className="h-4 w-4 text-muted-foreground" />;
  ...
  return (
    <TableRow>
      ...
      <TableCell><Badge variant="secondary">{player.position}</Badge></TableCell>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <div className="grid gap-3">
                ...
                <Input type="number" value={form.jerseyNumber} onChange={(e) => setForm({ ...form, jerseyNumber: parseInt(e.target.value || "0") })} />
                ...
                <div className="flex items-center gap-2 pt-2">
                  <Checkbox id={`paid-${player.id}`} checked={form.paid} onCheckedChange={(v) => setForm({ ...form, paid: !!v })} />
                  <label htmlFor={`paid-${player.id}`} className="text-sm">Pago</label>
                </div>
