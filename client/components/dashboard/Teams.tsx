...
import { type Team, useApp } from "@/store/app";
...
function TeamForm() {
	...
	const onSubmit = (e: React.FormEvent) => {
		...
		dispatch({
			type: "ADD_TEAM",
			payload: { name: name.trim(), color, capacity },
		});
		setName("");
		setColor("#22c55e");
		setCapacity(8);
		...
		return (
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{Array.from({ length: 6 }).map((_, i) => (
					<div
						key={`sk-team-${i}`}
						className="overflow-hidden rounded-md border bg-card p-4 shadow-sm"
					>
		...
		<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{teamsCat.map((t) => (
				<TeamCard
					key={t.id}
					team={t}
					assignedCount={assignments[t.id]?.length || 0}
					onDelete={() => dispatch({ type: "DELETE_TEAM", payload: { id: t.id } })}
				/>
			))}
		</div>
	);
