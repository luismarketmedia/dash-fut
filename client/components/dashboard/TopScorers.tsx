...
export function TopScorersSection() {
  ...
  const rows = useMemo(() => {
    const totals = new Map<string, { playerId: string; name: string; teamName: string; teamColor: string; goals: number }>();
    const teamOf = (pid: string) => {
      for (const [tid, ids] of Object.entries(state.assignments)) {
        if (ids.includes(pid)) return state.teams.find((t) => t.id === tid) || null;
      }
      return null;
    };
    for (const m of state.matches) {
      for (const [pid, ev] of Object.entries(m.events)) {
        ...
        const k = pid;
        const cur = totals.get(k) || { playerId: pid, name: pl.name, teamName: team?.name || "-", teamColor: team?.color || "#999", goals: 0 };
        cur.goals += ev.goals;
        totals.set(k, cur);
      }
    }
              ...
              <TableRow key={r.playerId}>
                ...
                <TableCell>
                  <span className="inline-flex items-center gap-2">
                    <span className="inline-block h-3 w-3 rounded" style={{ backgroundColor: r.teamColor }} />
                    {r.teamName}
                  </span>
                </TableCell>
