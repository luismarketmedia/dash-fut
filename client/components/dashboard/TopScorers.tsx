...
export function TopScorersSection() {
	...
	const rows = useMemo(() => {
		...
		>();
		const teamsSet = new Set(state.teams.filter((t) => t.categoryId === cat).map((t) => t.id));
		const teamOf = (pid: string) => {
			for (const [tid, ids] of Object.entries(state.assignments)) {
				if (ids.includes(pid) && teamsSet.has(tid))
					return state.teams.find((t) => t.id === tid) || null;
			}
			return null;
		};
		for (const m of state.matches.filter((m) => m.categoryId === cat)) {
			for (const [pid, ev] of Object.entries(m.events)) {
				const pl = state.players.find((p) => p.id === pid && p.categoryId === cat);
				if (!pl) continue;
	...
	}, [state.matches, state.players, state.assignments, state.teams, cat]);
